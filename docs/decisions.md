...
| D37 | ~~Separate short domain from the canonical origin~~ **SUPERSEDED by D69** | A short-link compromise could not serve a forged policy from the trusted origin | T8 | — | — | See D69 |
...
| D69 | Short links live on the product domain (`rightsroot.com/p/{code}`), not a separate one | Only two domains were acquired: `rightsroot.com` for the product and `rightsroot.org` for the open specification. A third short domain would be bought purely for origin separation | T8 | **Yes** | — | Buy a third domain — restores D37's separation, at the cost of another name to hold and renew. **This is a real, if small, regression:** a short-link compromise now shares an origin with the canonical policy. Mitigated by D59 — the QR carries the policy digest in its fragment, so a substituted document is detectable from a printed card regardless of which origin served it |
| D70 | Product is **RightsRoot**; protocol is **PRM**; specification lives at `rightsroot.org/spec/prm` | Separating the platform from the open standard keeps the standard credible: an implementation nobody operates can still be trusted, and a competitor can implement PRM without depending on us | T4 | **Yes** | — | One brand for both — makes the "provider need not be trusted" claim harder to believe, because the protocol and the operator look like the same thing |
| D71 | The `@context` namespace moved from `prm.dev` to `rightsroot.org/spec/prm/ns/v1` | `prm.dev` was never controlled by this project. A JSON-LD context URL pointing at a domain a third party could register is an integrity problem, not a cosmetic one | **T4**, T8 | **Yes** | JSON-LD context URI | Keep `prm.dev` — leaves a dangling reference to a domain anyone could acquire and populate |
| D72 | The browser app's CSP is a **per-request nonce** (`script-src 'nonce-…' 'strict-dynamic'`), and every page renders at request time | Next.js emits its bootstrap and RSC payload as inline scripts. A static `script-src 'self'` blocked them: the HTML painted, nothing hydrated, every button was dead. A nonce lets the framework mark its own scripts executable without `unsafe-inline`; a nonce differs per response, so pages carrying one cannot be prerendered | **T4** (an XSS beside an unlocked key) | **Yes** | CSP3 nonces, `strict-dynamic` | `'unsafe-inline'` with static pages — keeps CDN caching of HTML and reopens the injection surface the policy exists to close. The cost accepted here is that Vercel serves each page from a function rather than the edge cache; for a signing tool with a handful of pages, that is negligible next to the property it buys |
| D73 | JSON Schema validators are **compiled ahead of time**; the browser bundle contains no Ajv compiler | Ajv's default mode builds each validator with `new Function`, which needs `script-src 'unsafe-eval'`. Granting that would hand any XSS `eval` next to an unlocked signing key. Standalone compilation runs the same logic at build time and ships plain module code | **T4** | **Yes** | Ajv standalone | Add `'unsafe-eval'` to the CSP — the tempting fix, and the worst available one. The build fails if a `require`/`eval` survives into the generated validators (`packages/schema/scripts/compile-validators.mjs`) |
| D74 | Restore **adopts the published key event log**; it never regenerates a genesis event | The account id is the digest of the genesis event, which carries a timestamp and device label. Rebuilding it from the phrase on another day yields a different id — a new identity that looks like a recovery. `adoptAccount` reproduces the keys from the phrase, takes the genesis from the published log (by handle, or from a `.prmproof` with the publisher gone), and refuses unless the seed provably controls the log head | T12 (loss) | **Yes** — MVP §5 criterion 7 | KERI inception semantics | `restoreAccount(phrase)` alone — the vault it produces cannot sign for the account it was meant to recover, and the user finds out at their next publish |
| D75 | A signed tree head is emitted on **every append**; RFC 3161 tokens are requested **hourly over the newest head**, by cron and opportunistically by any request that finds it unanchored after an hour | A head per append means a leaf has an inclusion proof the moment it lands, so the client attaches it immediately and only the token is deferred. Anchoring stays hourly because the batching is a privacy control (docs/08 §4): the authority sees one request an hour whose contents do not vary with who published. The opportunistic path makes "a token within the hour" (MVP §5 criterion 3) hold even if the cron misses | T4, **T11** | **Yes** | RFC 3161, RFC 6962 | Token per publish — fast, and a permanent per-user timing side channel at the authority |
| D76 | The token imprint is `SHA-256(rootHash bytes)`, and the verifier checks **binding** (token ↔ tree head ↔ leaf ↔ entry ↔ policy) but deliberately **not** the authority's signature | Binding is structural and needs no trust decision; a token that merely sits in a bundle proves nothing, and before this the verifier counted tokens without reading them. The CMS signature needs the authority's chain and a decision about which roots to accept — that belongs with the person verifying, using `openssl`, with the chain the operator archived at acquisition | T4, T8 | **Yes** | RFC 3161, CMS (RFC 5652) | Validate the signature in `@prm/verify` — pulls X.509 path validation and a root store into a package whose value is having no dependencies, and makes a trust decision on the reader's behalf |
| D77 | The server links a policy digest to the **entry digest and leaf** the client appended, so the public page and `/u/{handle}/log.json` can serve the evidence to a third party | The log holds only leaf hashes (D23) and the server never sees the ledger entry; without the link it could not say which leaf is a given policy's. Both digests are already public, and the link reveals only what `publishedAt` does | T5 | **Yes** | — | Make the reader ask the issuer for the entry every time — the evidence a records officer needs would depend on the issuer being reachable |
| D78 | The CLI is published as **`@rightsroot/prm-cli`**, a single bundled file with **no dependencies**; the workspace packages stay unpublished under their internal `@prm/*` names | The `@prm` npm scope belongs to an unrelated deprecated package, and every verification instruction said `npx @prm/cli` — which ran that stranger's code silently. A published package listing `@prm/*` dependencies would have resolved to the same stranger. Bundling makes the published artifact exactly what runs, verifiable by its own hash | **T4** (supply chain), credibility | **Yes** | — | Publish all four packages under a new scope — four things to version and a dependency graph a records officer's `npx` has to resolve correctly; the bundle removes the graph entirely |
| D79 | Storage is **namespaced by environment in code**: a preview deployment uses `preview_*` relations and `preview/` blob paths, decided by `VERCEL_ENV` alone | A preview was given production credentials and published a test policy into production (`/u/user0001`). Configuration is the wrong place to hold that line — it is one env-scoping mistake from failing again. Namespacing in the adapters means the same DATABASE_URL and BLOB_READ_WRITE_TOKEN cannot reach a production row or object from a preview, and production never reads a preview relation. The full storage and log contracts run against the namespaced adapters, so behaviour is identical | **T4** (evidence integrity), operability | **Yes** | — | Separate credentials per environment only — still the right thing to do (a Neon branch per preview), but a floor that depends on nobody ever scoping a variable wrong is not a floor |
